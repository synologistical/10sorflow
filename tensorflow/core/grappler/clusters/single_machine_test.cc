...
TEST_F(SingleMachineTest, ReleaseMemoryAfterDestruction) {
  ...
  const uint64_t kMaxMemoryUsage = 2048;
  // There might be a bit memory used before session's running anything.
  EXPECT_LT(device_peak_memory_before.begin()->second, kMaxMemoryUsage);
  ...
  EXPECT_LT(device_peak_memory_before.begin()->second, kMaxMemoryUsage);
  EXPECT_LT(device_peak_memory_after.begin()->second, kMaxMemoryUsage);
}
